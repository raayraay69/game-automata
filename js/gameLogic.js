...
function selectTopic(topic) {
    ...
    if (quizData[currentTopic][0].visualizationType === 'DFA' || quizData[currentTopic][0].visualizationType === 'NFA') {
        window.automataGalaxy.showDFANFAVisualization(quizData[currentTopic][0], true, true);
    }
}
...
function runSimulation() {
    ...
    let currentState = 'q0';
    let step = 0;
    window.automataGalaxy.automaton.position.copy(window.automataGalaxy.states[0].position);
    ...
    const interval = setInterval(() => {
        if (step < input.length) {
            ...
            let nextTransitions = window.automataGalaxy.transitions.filter(t => t.from === currentState && (t.symbol === symbol || (currentTopic === 'NFA' && t.symbol === 'ε')));
            if (nextTransitions.length > 0) {
                const nextTransition = nextTransitions[0];
                currentState = nextTransition.to;
                const nextPosition = window.automataGalaxy.states.find(s => s.name === currentState).position;
                window.automataGalaxy.enhancedMoveAutomaton(nextPosition);
            ...
            const finalState = window.automataGalaxy.states.find(s => s.name === currentState);
...
function showVisualDemonstration(question, isCorrect, isTutorial = false) {
    if (question.visualizationType === 'DFA' || question.visualizationType === 'NFA') {
        window.automataGalaxy.showDFANFAVisualization(question, isCorrect, isTutorial);
    } else if (question.visualizationType === 'Pumping') {
        window.automataGalaxy.showPumpingVisualization(question, isCorrect);
...
function init() {
    window.automataGalaxy.initVisualization();
    loadSounds();
    updateHUD();
...
// Store sounds in the global namespace
window.automataGalaxy = window.automataGalaxy || {};
window.automataGalaxy.sounds = sounds;
